...
# Description :   A tool to create rectangles in arbitrary orientations.
...
def create_rectangle
    ...
    if( @pts[0] != @pts[3] )
        new_face = Sketchup.active_model.active_entities.add_face @pts

        new_group = group = Sketchup.active_model.active_entities.add_group( new_face )

        new_group.name = "hazard_dangerZone"
